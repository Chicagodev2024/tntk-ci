...
This repository is an integral part of the learnin project designed for training purposes. It contains the application code and its CI for integration into the infrastructure described in this repository.
...
### Idempotency
We use AWS SQS as a simple and convenient solution to help implement Idempotency using fully managed ordered message queues.
After the backend receives a request from the user with the requested URL, it will send it to the SQS message queue. Lambda function as an idempotent order handler is connected to this queue which processes all incoming messages in order of the FIFO principle. That event chain will prevent unnecessary duplicated stateful operations.
